...
public class TestTablePagination extends Application {
    ...
    private static TestTablePagination instance;

    public TestTablePagination() {
        instance = this;
    }

    public synchronized static TestTablePagination getInstance() {
        if (instance == null) {
            new Thread(() -> {
                // Have to run in a thread because launch doesn't return
                Application.launch(TestTablePagination.class);
            }).run();
            while (instance == null) {
                try {
                    Thread.sleep(100);
    ...
    private final static int dataSize = 20;
    ...
    public static void main(String[] args) throws Exception {
        TestTablePagination instance = getInstance();
        instance.update();
    }

    private void update() {
        final List<Sample> data = new ArrayList<>(dataSize);
        for (int i = dataSize; i < dataSize + 20; i++) {
            data.add(new Sample(i, "ufoo " + i, "ubar " + i));
        }
